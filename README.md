...
> 🎚️ **A/V sync.** The analog camera warms up 1–3 s after the mic is already streaming, so the recorder drops the leading audio that has no video yet and re-stamps the rest to the first video frame — the picture and sound stay aligned. The dropped amount is measured per session, so it adapts to a variable warm-up rather than assuming a fixed lag.
...
**❓ A recorded clip is missing / unplayable and no notification arrived**
- Fixed in **v0.9.2**. Symptom in the logs: `Error finalizing recording` followed by `av.error.ArgumentError: … non monotonically increasing dts to muxer in stream 0`. The MP4 was never finalized (no `moov` atom), so it can't be played and the folder-watcher never fires a "closed" event.
- Cause: an occasional non-advancing frame timestamp from Ring's WebRTC feed (jitter / RTP retransmission / repeated key-frame) turned into a duplicate DTS at the muxer. v0.9.2 forces the recorded video's PTS strictly increasing, so the muxer always closes the file cleanly. Update to v0.9.2+.
...
| **v0.9.1** | 🎚️ **A/V sync for recorded audio** | Fixes the picture lagging the sound in recorded clips. Ring's analog camera warms up ~1–3 s **after** the microphone is already streaming, and aiortc normalizes each track's PTS independently, so the leading audio had no matching video and everything looked delayed. The recorder now **drops the pre-warm-up audio and re-stamps it to the first video frame**, aligning the two. The offset is measured per session (adapts to a variable warm-up), not a fixed value. |
| **v0.9.2** | 🩹 **Muxer fix — recordings no longer lost to a duplicate DTS** | Fixes intermittent clips that were **left unplayable** (and produced **no notification**). Ring's WebRTC feed occasionally emits a frame whose timestamp doesn't advance (jitter, an RTP retransmission, a repeated key-frame); aiortc re-encodes frames in arrival order, so that stalled PTS became a **duplicate DTS** the MP4 muxer rejects on flush (`non monotonically increasing dts to muxer`), aborting `recorder.stop()` and leaving the file with **no `moov` atom**. The recorder now passes the video through a guard that **forces strictly increasing PTS** (nudging any stalled timestamp forward one ~11 µs tick — imperceptible, no frame dropped or reordered), so the muxer always finalizes cleanly. |
